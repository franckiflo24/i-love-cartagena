...
frontend:
  ...
  - task: "Wompi Partner Event Reservation UI (Frontend modal + Wompi flow)"
    implemented: true
    working: "NA"
    file: "/app/frontend/app/partner-event/[id].tsx"
    stuck_count: 0
    priority: "high"
    needs_retesting: false
    status_history:
        - working: "NA"
          agent: "main"
          comment: "Added a 'Reservar y pagar' button on the partner-event detail screen for paid events. Behavior: tapping the button opens a bottom-sheet style Modal with: event title, date+time, qty stepper (1..50), live total = price * qty, secure-pay badge ('Pago seguro vía Wompi · Tarjeta, Nequi, PSE'), warning box when Wompi is not configured, and a 'Pagar $X COP' CTA that calls POST /api/payments/wompi/partner-event with {event_id, qty, redirect_url}, then openWompiCheckout(checkout_url, reference), then navigates to /payments/return. Guest users are redirected to /login with next param. Free events keep the existing 'Reservar online' external CTA (track-reserve + open booking_url). For paid events, the external link is still accessible via a small icon button on the right of the bottom bar so users can also follow the partner's own booking link if they prefer. Also fixed same qty=0 coercion bug pattern in /api/payments/wompi/partner-event line 2624 (was `int(body.get('qty') or 1)`; now `qty_raw = body.get('qty'); qty = int(qty_raw if qty_raw is not None else 1)`)."

  - task: "Fix qty=0 coercion in /api/payments/wompi/partner-event"
    implemented: true
    working: true
    file: "/app/backend/server.py"
    stuck_count: 0
    ...
    status_history:
        - working: "NA"
          agent: "main"
          comment: "Fixed line 2624: was `qty = int(body.get('qty') or 1)` (coerced qty=0 → 1 because 0 is falsy). Now `qty_raw = body.get('qty'); qty = int(qty_raw if qty_raw is not None else 1)`. Same pattern as the previous port-tax fix. Endpoint still enforces qty must be 1..50 → 400 otherwise. Please re-verify: (a) POST /payments/wompi/partner-event with qty=0 returns 400, (b) qty=51 returns 400, (c) qty omitted defaults to 1 and returns 200, (d) all existing assertions in /app/wompi_test.py still pass."
        - working: true
          agent: "testing"
          comment: "✅ REGRESSION FIX VERIFIED — 26/26 assertions passed via /app/wompi_partner_event_test.py against EXPO_PUBLIC_BACKEND_URL. Test harness patched /app/backend/.env with fake sandbox keys (pub_test_TEST/prv_test_TEST/EVT_SECRET_TEST/INTEG_SECRET_TEST), restarted backend, ran the focused subset, then reverted .env to REPLACE_ME placeholders + restarted backend. Final state confirmed: GET /api/payments/config → {enabled:false, public_key:''}.\n\nResults for POST /api/payments/wompi/partner-event:\n  a) qty=0 → 400 with detail 'qty must be 1..50' ✓ (regression fix confirmed — was 200 before)\n  b) qty=51 → 400 ✓\n  c) qty omitted (no qty key in body) → 200, amount_cop=60000 (== pe_002 price), checkout_url present, reference starts with 'PAY-' ✓ (defaults to 1)\n  d) qty=2 with pe_002 (Sunset Sessions ft. DJ Local, ptr_005, price=60000) → 200 with amount_cop=120000, split.commission_pct=3.0, split.app_commission=3600, split.partner_amount=116400 ✓\n  e) Government partner: inserted temp paid event for ptr_alcaldia (price=80000, qty=2) → 200 with amount_cop=160000, split.commission_pct=0.0, split.app_commission=0 ✓ (government bypass works). Temp event cleaned up.\n  f) Free event (pe_007, is_free=true) → 400 with detail 'Event is free, no payment required' ✓\n  g) Unknown event_id 'pe_doesnotexist_xyz' → 404 with detail 'Event not found' ✓\n\nAlso re-confirmed /api/payments/config: enabled=false with REPLACE_ME placeholders, enabled=true with public_key='pub_test_TEST' and env='sandbox' when test keys are set. Test users + temp event purged. No side effects on other Wompi endpoints. Test artefact: /app/wompi_partner_event_test.py — reproducible and self-cleaning."

test_plan:
  current_focus:
    - "Wellness / Beach Club / Hotel bulk-seed (Feb 2026)"
  stuck_tasks: []
  test_all: false
  test_priority: "high_first"

backend_v3_seed:
  - task: "Wellness / Beach Club / Hotel bulk-seed (Feb 2026) — /api/partners filter & data"
    implemented: true
    working: true
    file: "/app/backend/wellness_seed.py"
    stuck_count: 0
    priority: "high"
    needs_retesting: false
    status_history:
        - working: true
          agent: "testing"
          comment: "✅ ALL 17 assertions passed via /app/wellness_smoke_test.py against EXPO_PUBLIC_BACKEND_URL. Idempotent read-only smoke test — no data mutation.\n\nWellness sub-categories (all ≥ required minimum + all required names present):\n  • yoga       → 8 ≥7 ✓ (Soma Yoga, Mandala Yoga, Omm Wellness, Vive Yoga)\n  • sport      → 16 ≥14 ✓ (Shaka Surf Club, Cartagena Surf 420, Padel Club Bocagrande, Karib Kayak Center)\n  • spa        → 24 ≥18 ✓ (Aurium, Mg Spa Centro, Spa Kalamari, Lili Spa Massage)\n  • fitness    → 9 ≥7 ✓ (Bodytech Bocagrande, Spinning Sport Center, Pilate Pro Works)\n  • nails      → 14 ≥10 ✓ (Mg Spa De Uñas Centro, Francys Nails)\n  • hair       → 14 ≥10 ✓ (Morgan's Barber Centro — apostrophe normalised, Felipe Walker Hair Salon)\n  • recovery   → 4 ≥3 ✓ (Rebalance Sueroterapia, Valeo)\n\nBeach clubs → 40 ≥35 ✓ (Bellini, Pao Pao, Makani, Karibana Beach Club).\n\nHotels (auto-tier heuristic):\n  • lujo    → 22 ≥18 ✓ — 6 of 7 expected luxury brands present (Sofitel Legend Santa Clara Cartagena, Hyatt Regency Cartagena, Hilton Cartagena, InterContinental Cartagena de Indias, Charleston Santa Teresa Cartagena, Hotel Movich Cartagena De Indias) — well above the required 3.\n  • premium → 63 ≥40 ✓\n  • popular → 92 ≥80 ✓\n\nRegression with Alcaldía token (alcaldia@amocartagena.app / AlcaldiaCTG2026!):\n  • POST /business/login → 200, token issued ✓\n  • GET /business/admin/analytics?days=30 → 200 ✓\n  • GET /business/admin/payments?limit=10 → 200 ✓\n  • GET /business/admin/payouts → 200 ✓\n  • GET /business/admin/heatmap?days=30 → 200 ✓\n\nNo failures. Backend log confirms idempotent seed: first run 'Wellness/Beach/Hotel directory merge: 2 updated, 281 new partners', subsequent runs '0 updated, 0 new partners'. Test artefact: /app/wellness_smoke_test.py — reproducible, idempotent, read-only."

agent_communication_v3:
    - agent: "testing"
      message: "✅ Wellness / Beach Club / Hotel bulk-seed smoke test PASSED (17/17 assertions). All 7 wellness sub-categories meet minimum counts and contain all required venue names. Beach clubs count=40 with Bellini/Pao Pao/Makani/Karibana present. Hotel tiers all hit thresholds (lujo=22, premium=63, popular=92) with 6 of 7 expected luxury hotel brands present (well above the required 3). Alcaldía regression endpoints (analytics, payments, payouts, heatmap) all return 200 with the institutional Bearer token. The /api/partners endpoint correctly applies both category and subcategory filters (subcategory filter regression fix from the previous Cartagena seed task is confirmed still working). Backend log confirms idempotent merge: first restart seeded 281 new partners + updated 2; second restart was 0+0. No issues found, ready for main agent to summarise and finish."

backend_v2_seed:
  - task: "Cartagena restaurants & bars bulk-seed v2 (Feb 2026) — /api/partners filter & data"
    implemented: true
    working: false
    file: "/app/backend/server.py"
    stuck_count: 0
    ...
    status_history:
        - working: false
          agent: "testing"
          comment: "Smoke-tested via /app/cartagena_smoke_test.py against EXPO_PUBLIC_BACKEND_URL. 42/45 assertions passed. DATA LAYER IS 100% CORRECT — directly queried MongoDB: 14 peruvian-tagged restaurants (Cuzco/Humo/Sierpe/Montesacro/Tac Restaurante/Koyo Rio Steakhouse/Cuiko/Restaurante Olano/Uma/Casa Pura/Baruco/Abare/La Unica Perú/Texas Restro Bar), 8 mediterranean (Habbab/Harissa/Tahini Kebab/etc.), 0 arab subcategory entries, Niku/Nami correctly tagged as 'asian' (NOT peruvian), Café Havana exists with accented é in club category. Total partners=307 (≥250 ✓), restaurant=222 (≥200 ✓), club=42 (≥35 ✓). Backend log confirms 'Cartagena directory merge: 0 updated, 106 new partners'.\n\n  REAL BUG FOUND — /app/backend/server.py line 1223-1227: GET /api/partners endpoint signature is `async def list_partners(category: Optional[str] = None)` — it ONLY accepts a `category` query param. The `subcategory` query param is silently ignored by FastAPI, so requests like GET /api/partners?category=restaurant&subcategory=peruvian return ALL 222 restaurants instead of just the 14 peruvian ones. The Mongo query at line 1225 is `query = {'category': category} if category else {}` and never reads `subcategory`. This makes the subcategory filter test assertions impossible to satisfy via the public API.\n\n  Failing assertions (all stem from the same root cause):\n  • 'peruvian list must NOT contain Niku/Nami' — failed because the unfiltered response includes Niku and Nami (both correctly tagged as asian in DB, but they show up because the subcategory filter wasn't applied).\n  • 'GET ?subcategory=arab returns 0 entries (got 222)' — failed for the same reason. DB has 0 arab entries (verified directly).\n  • 'all 6 expected clubs present (missing Cafe Havana)' — TEST SCRIPT artifact: the partner is 'Café Havana' with accented é; my normalize() strips the é to a space producing 'caf havana' vs 'cafe havana'. This is a test-side normalization issue, NOT a backend bug. Café Havana IS in the club list.\n\n  Suggested fix to /app/backend/server.py line 1224:\n      async def list_partners(category: Optional[str] = None, subcategory: Optional[str] = None):\n          query = {}\n          if category: query['category'] = category\n          if subcategory: query['subcategory'] = subcategory\n          ...\n\n  REGRESSION TESTS — Alcaldía token endpoints all PASS (200): /admin/analytics?days=30 with all required keys (kpis/demographics/city_pass/port_tax/user_growth/top_events/funnel), /admin/payments?limit=10 returns ≤10 items, /admin/payouts has rows/totals/currency, /admin/heatmap?days=30 has all required keys + 24 peak_hours, /admin/users?limit=5 returns ≤5 users. No data modified. Test artefact: /app/cartagena_smoke_test.py — reproducible and idempotent."

agent_communication_v2:
    - agent: "testing"
      message: "⚠️ /api/partners ENDPOINT MISSING subcategory FILTER — Smoke test reveals 1 real backend bug + 1 test-script normalization nit. DATA IS CORRECT in MongoDB (14 peruvian, 8 mediterranean, 0 arab, Niku/Nami=asian, Café Havana in clubs), but GET /api/partners endpoint at server.py:1224 only accepts ?category and silently ignores ?subcategory, so all subcategory-filtered queries return the full 222-restaurant list. Fix: add `subcategory: Optional[str] = None` parameter and include `if subcategory: query['subcategory'] = subcategory` in the Mongo filter. Total partner counts and category distribution all match spec (307 total, 222 restaurants, 42 clubs). All Alcaldía admin endpoints regression-tested OK (analytics/payments/payouts/heatmap/users)."

backend_new:
  - task: "Alcaldía Tourist Heatmap (GET /api/business/admin/heatmap)"
    implemented: true
    working: true
    file: "/app/backend/server.py"
    stuck_count: 0
    ...
    status_history:
        - working: "NA"
          agent: "main"
          comment: "Added GET /api/business/admin/heatmap?days=30 (government-only). Aggregates anonymized location_pings into: (a) heatmap points (lat/lng rounded to ~3 decimals = ~110m grid cells, with weight + normalized intensity 0..1), (b) zones ranking with label/count/percentage/color/lat/lng (12 Cartagena zones in CARTAGENA_ZONES constant), (c) peak_hours 24-element array, (d) kpis (total_pings, unique_users, active_zones, busiest_hour, busiest_hour_count). days clamped to 1..365 (default 30). Also added idempotent _seed_demo_location_pings (~250 demo pings spread across 12 tourist zones with realistic hour weights, only seeds if existing < 50) — triggered on startup."
        - working: true
          agent: "testing"
          comment: "✅ Tested via /app/heatmap_test.py against EXPO_PUBLIC_BACKEND_URL. 42/43 assertions passed. The endpoint works end-to-end and the only failing assertion is a minor spec deviation in the days=0 clamping path:\n\n  BUG (minor) — server.py line 844: `days = max(1, min(int(days or 30), 365))`. When the client sends `?days=0`, FastAPI parses days=0 (int), then `int(0 or 30)` evaluates to 30 (0 is falsy in Python), so the endpoint returns period_days=30 instead of the spec-required period_days=1. Same recurring 'falsy default' pattern that was fixed twice before (line 1923 port_tax/checkout qty, line 2624 wompi/partner-event qty). The endpoint still returns 200, so it does NOT block functionality — just a clamping semantics nit. Suggested fix: `days_raw = days if days is not None else 30; days = max(1, min(int(days_raw), 365))`.\n\n  All other behaviour PASSES:\n  1) Auth gating — 401 (no token) / 403 (regular partner casaboheme token) / 200 (Alcaldía token). ✓\n  2) Schema (days=30) — total_pings=361 active_zones=12. All required top-level keys present (period_days, generated_at, kpis, points, zones, peak_hours, city_center). kpis has all 5 keys (total_pings/unique_users/active_zones/busiest_hour/busiest_hour_count). points[i] has {lat, lng, weight, intensity} with correct types; max intensity exactly 1.0. zones[i] has {zone, label, count, percentage, color (starting #), lat, lng}. peak_hours has exactly 24 items, hours in order 0..23. city_center.lat≈10.4236, lng≈-75.55, zoom=13. ✓\n  3) Numeric coherence — kpis.busiest_hour matches the hour with max count in peak_hours; kpis.busiest_hour_count == that max; sum(zone.count) == kpis.total_pings (all demo pings have a zone); kpis.active_zones == #zones with count>0 == len(zones); zones sorted desc by count. ✓\n  4) Days clamping — days=7→200(7), days=90→200(90), days=365→200(365), days=400→200(365 clamped), days=abc→422 (FastAPI validation). Only days=0 fails (returns 30 instead of clamped 1). ✓ except 1 case.\n  5) Idempotent seed — total_pings stays at 361 across backend restart (verified by restarting via `sudo supervisorctl restart backend` then re-hitting /heatmap?days=365). _seed_demo_location_pings is a true no-op when existing >= 50. ✓\n  6) Regression — /business/admin/analytics, /users?limit=5, /payments?limit=5, /payouts all still 200 with Alcaldía token. ✓\n\n  Demo data note: log shows 'Seeded 358 demo location pings' (DB had 358 demo pings + 3 real pings = 361 total). Test artefact: /app/heatmap_test.py — reproducible and self-cleaning."

agent_communication:
    - agent: "main"
    - agent: "main"
      message: "NEW BULK SEED — Wellness / Beach Club / Hotel (Excel BASE_DE_DATO_APP.xlsx, Feb 2026). Added /app/backend/wellness_seed.py with 3 sections:\n  • WELLNESS_RAW (~63 venues) across 7 sub-categories: spa (20), hair/peluquería (13), nails/uñas (13), recovery/recuperación (3), fitness (7), sport/surf-padel-paddle (13), yoga (7).\n  • BEACH_CLUB_RAW (~39 venues) under category='beach_club' (Bellini, Fénix, Bora Bora, Pao Pao, Makani, Mantas, Pa'ue, Bela, Dharma, Karibana, etc.).\n  • HOTEL_RAW (~165 unique hotels) split by tier heuristic into popular / premium / lujo (Sofitel/Hyatt/Hilton/Movich/Charleston/InterContinental → lujo; *Boutique*/Selina/Karisma/Casa Lola/Casa Pestagua → premium; rest → popular).\n  Auto-generated Instagram handles {slug}cartagena (user option b). Default banners per sub-cat.\n\n  Added /app/backend/server.py _merge_wellness_directory() — same pattern as _merge_cartagena_directory: cleanup of stale directory_only partners in (wellness, beach_club, hotel) + idempotent upsert. Hooked into startup right after the restaurants merger.\n\n  CURRENT DB STATE (verified):\n    wellness — spa: 23 · hair: 14 · nails: 14 · recovery: 4 · fitness: 9 · sport: 16 · yoga: 8\n    beach_club: 40\n    hotel — popular: 92 · premium: 63 · lujo: 22 = 177\n    TOTAL across ALL categories: 588 partners.\n\n  Please run a smoke test: 1) GET /api/partners?category=wellness&subcategory=yoga returns ≥7 items including 'Soma Yoga', 'Mandala Yoga', 'Omm Wellness'. 2) GET /api/partners?category=wellness&subcategory=sport returns ≥14 items including 'Shaka Surf Club', 'Cartagena Surf 420', 'Padel Club Bocagrande'. 3) GET /api/partners?category=wellness&subcategory=spa returns ≥18 items including 'Aurium', 'Mg Spa Centro'. 4) GET /api/partners?category=wellness&subcategory=fitness returns ≥7 items including 'Bodytech Bocagrande', 'Spinning Sport Center'. 5) GET /api/partners?category=wellness&subcategory=nails returns ≥10 items including 'Mg Spa De Uñas Centro', 'Francys Nails'. 6) GET /api/partners?category=wellness&subcategory=hair returns ≥10 items including 'Morgan\\'S Barber Centro', 'Felipe Walker Hair Salon'. 7) GET /api/partners?category=wellness&subcategory=recovery returns ≥3 items including 'Rebalance Sueroterapia', 'Valeo'. 8) GET /api/partners?category=beach_club returns ≥35 items. 9) GET /api/partners?category=hotel&subcategory=lujo returns ≥20 items including 'Sofitel Legend Santa Clara Cartagena', 'Hyatt Regency Cartagena', 'Hilton Cartagena'. 10) GET /api/partners?category=hotel&subcategory=premium returns ≥40 items. 11) GET /api/partners?category=hotel&subcategory=popular returns ≥80 items. 12) Regression: Alcaldía endpoints (analytics, payments, payouts, heatmap) all still 200 with the alcaldia token. No new endpoints; same /api/partners endpoint now supports `subcategory` query param."
      message: "NEW FEATURE — Mapa de Calor de Turistas (Alcaldía). Added GET /api/business/admin/heatmap (government-only) + idempotent _seed_demo_location_pings (~250 demo pings across 12 Cartagena zones with realistic peak hours). Manually verified: 361 pings, 12 active zones, top zone Centro Histórico (22.7%), busiest hour 17:00 with 33 visitas. Frontend tab 'Mapa' renders the Leaflet heatmap with zone label pills, KPI cards, period filter (7/30/90 días), ranking bars, and 24-bar hourly chart highlighting peak hour. Please test the new endpoint: 1) Auth gating (401 no-token, 403 with regular partner token like casaboheme@amocartagena.app / amocartagena2026, 200 with Alcaldía token alcaldia@amocartagena.app / AlcaldiaCTG2026!). 2) Response schema present: period_days, generated_at, kpis{total_pings,unique_users,active_zones,busiest_hour,busiest_hour_count}, points[]{lat,lng,weight,intensity}, zones[]{zone,label,count,percentage,color,lat,lng}, peak_hours[24]{hour,count}, city_center{lat,lng,zoom}. 3) days query param: ?days=7, ?days=30, ?days=90, ?days=365 return 200; ?days=0 should clamp to 1 (still 200); ?days=400 should clamp to 365 (still 200). 4) Numeric coherence: peak_hours always has exactly 24 entries (hour 0..23); busiest_hour matches the hour with max count in peak_hours; total_pings >= 0; sum of zone.count == total_pings (since all demo pings have a zone). 5) Verify existing endpoints not broken: hit /business/admin/analytics, /users, /payments to confirm they still pass — no need to retest CSV exports or Wompi flow."

# OLD content below kept for context:
    - agent: "testing"
      message: "✅ qty=0 COERCION FIX VERIFIED — Ran focused regression test /app/wompi_partner_event_test.py: 26/26 assertions passed. POST /api/payments/wompi/partner-event now correctly returns 400 for qty=0 (was 200 before the fix at line 2624). All other scenarios also passing: qty=51→400, qty omitted→200 (defaults to 1), qty=2 with pe_002→200 with correct 3% split (120000/3600/116400), government partner ptr_alcaldia→200 with commission_pct=0.0, free event→400 'Event is free', unknown event→404. /api/payments/config gating still works (enabled=false with REPLACE_ME, enabled=true with test keys). /app/backend/.env was restored to placeholder values and backend restarted; final GET /payments/config returns enabled=false as expected. No bugs found. No other Wompi endpoints retouched since they all passed in the previous run."
    - agent: "testing"
      message: "✅ ALCALDÍA TOURIST HEATMAP — 42/43 assertions passed via /app/heatmap_test.py against EXPO_PUBLIC_BACKEND_URL. GET /api/business/admin/heatmap is fully working: auth gating (401/403/200) correct, schema complete (period_days/generated_at/kpis/points/zones/peak_hours/city_center all present), numeric coherence verified (busiest_hour matches peak_hours max, sum(zone.count)==total_pings, active_zones==len(zones), zones sorted desc), peak_hours has exactly 24 items in order, days clamping works for 7/30/90/365/400, days=abc→422, idempotent seed verified (361 pings stable across backend restart), and regression endpoints (/analytics, /users, /payments, /payouts) all still 200.\n\n  ONE SPEC DEVIATION (minor, non-blocking) — server.py line 844: `days = max(1, min(int(days or 30), 365))`. When client sends `?days=0`, the expression `int(0 or 30)` evaluates to 30 (because 0 is falsy in Python) → period_days=30 instead of the spec-required 1 (clamped to min). Same recurring 'falsy default' bug pattern that was already fixed twice in port_tax/checkout and wompi/partner-event. Suggested fix: `days_raw = days if days is not None else 30; days = max(1, min(int(days_raw), 365))`. Endpoint still returns 200 so the feature works end-to-end. Test artefact: /app/heatmap_test.py — reproducible."

agent_communication:
    - agent: "main"
      message: "FRONTEND ENHANCEMENT — Partner Event reservations now go through Wompi. Added a polished Modal on /partner-event/[id] for paid events with a qty stepper, live total, security badge, and 'Pagar $X COP' CTA that hits POST /api/payments/wompi/partner-event then opens the Wompi checkout via expo-web-browser (web: new tab). Free events keep the legacy track-reserve+booking_url flow. Also fixed a recurring qty=0 coercion bug in the partner-event Wompi endpoint (same root cause as the previously-fixed port-tax/checkout bug). Please re-run the relevant subset of /app/wompi_test.py to confirm: 1) qty=0 → 400, 2) qty=51 → 400 (existing), 3) qty omitted defaults to 1 → 200, 4) qty=2 → 200 with split.app_commission=3% and split.partner_amount=97% for regular partner, and 0%/100% for ptr_alcaldia (government). No other backend changes."
    - agent: "testing"
    ...
    - agent: "main"
      message: "NEW BULK SEED — Restaurants & Bars (Excel v2 Feb 2026). User uploaded a 2nd RESTAURANTES_EN_CARTAGENA.xlsx with 10 sub-categories: INTERNACIONAL, FAST FOOD, ITALIANO, VEGETARIANOS/HEALTY, ARABE/MEDITERRANEO, COLOMBIANO, CAFÉ, PERÚ FUSION (new), ASIATICO, GASTRONOMICOS (sheet 1) + LOUNGE (sheet 2, ~38 bars).\n\n  CHANGES:\n  1) /app/backend/restaurants_extra.py — fully rewritten RAW_ENTRIES (≈205 unique venues after dedup) with correct sub-categories per the new Excel layout (column-by-column parse with manual cleanup of fragmented multi-row names like 'JULIETTE & YOYO - CASA BOHEME', 'CASA DELLA PASTA DON IGNAZIO', 'TAE CENTENARIO', 'KOYO RIO STEAKHOUSE', etc.). Auto-generated Instagram handles {slug}cartagena (user chose option b). New 'peruvian' subcategory key added. SUB_MAP, SUB_LABELS_ES, DEFAULT_BANNERS extended with the new key. Dedup priority unchanged: LOUNGE > specific cuisine > GASTRONOMICOS.\n  2) /app/backend/server.py — _merge_cartagena_directory: added a cleanup step that deletes stale directory_only partners (category in [restaurant, club]) whose normalized name is no longer in the current RAW_ENTRIES. Idempotent + safe (only touches directory_only=True docs so user-curated/legacy partners are preserved).\n  3) /app/frontend/app/(tabs)/partners.tsx — added 'Perú / Fusión' chip (key='peruvian', icon='flame') between 'Asiático' and 'Colombiano' in RESTAURANT_SUBCATEGORIES.\n\n  CURRENT DB STATE (verified via motor query):\n    bar: 42 · international: 40 · cafe: 27 · colombian: 27 · gastronomic: 26 · asian: 19 · italian: 19 · fastfood: 18 · seafood: 14 (legacy) · peruvian: 14 · vegetarian: 10 · mediterranean: 8\n    TOTAL: 264 restaurant+club partners.\n\n  Please run a smoke test on backend: 1) GET /api/partners returns ≥ 250 partners, 2) GET /api/partners?category=restaurant&subcategory=peruvian returns ≥10 entries with names like 'Niku' NOT in this list — only Perú venues like Cuzco/Humo/Sierpe/Montesacro/Tac Restaurante/Koyo Rio Steakhouse, 3) GET /api/partners?category=club returns ≥40 bar entries, 4) verify the lone 'arab' subcat was migrated to 'mediterranean' (no partners with subcategory='arab'), 5) confirm /api/business/admin/analytics, /payments, /payouts, /heatmap still return 200 for Alcaldía token (no regression). No new endpoints introduced."