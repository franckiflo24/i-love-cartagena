...
agent_communication:
    ...
    - agent: "main"
      message: "ADDED WHATSAPP LOGIN: Added 'Continuar con WhatsApp' button to login screen with the WhatsApp brand green (#25D366) and logo-whatsapp icon. Stack of methods is now: Google (primary orange) → WhatsApp (brand green) → Email (outline) → Apple (iOS only, black). WhatsApp button opens a bottom-sheet modal with: green circular icon header, name input + phone input (logo-whatsapp icon green, +57 placeholder, phone-pad keyboard), 'Crear cuenta' green save button. Creates a local profile with provider='whatsapp_local' in AsyncStorage (auto-restored on app reopen via updated AuthContext). Translations added for ES/EN/FR/PT (login_whatsapp, login_whatsapp_title, login_whatsapp_subtitle, login_phone_placeholder). Verified visually: button rendering with brand green + logo, modal opening with all expected fields."