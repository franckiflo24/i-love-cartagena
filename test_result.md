...
test_plan:
  current_focus:
    - "AI Concierge 'Amo' — Rich Recommendation Cards (verified visually)"
  stuck_tasks: []
  test_all: false
  test_priority: "high_first"
...
agent_communication:
    - agent: "main"
      message: "✅ AI CONCIERGE RICH CARDS — Verified visually via screenshot tool. The previous fork claimed the rich recommendation cards were applied to the frontend but the AssistantFab.tsx send() handler was discarding the `recommendations` array and MessageBubble had no rendering for them. Fixed both: (1) send() now persists `recommendations` from API response into the message state, (2) MessageBubble now renders a horizontal-scrollable row of RecommendationCard items above the action buttons. Each card shows: type badge (PARTNER/Evento with accent color), price_range badge ($$ / $$$ / $$$$), name, type/subcategory line, vibe (with sparkles icon), reason, optional address, and a CTA button 'Ver partner' / 'Ver evento' that wires through onAction → open_partner / open_event for direct deep-link to the partner/event detail screen. Also strengthened the system prompt in ai_agent.py: replaced the contradictory '2-3 recommendations' rule with explicit 'MÍNIMO 5 TARJETAS, apuntá a 6-8' when there are enough relevant candidates. Verified with query 'Quiero comer italiano hoy, dame al menos 5 opciones': 5 cards rendered scrollably (Norma $$$, Benitto $$, Fornaio $$, Casa della Pasta Don Ignazio $$, Di Silvio Trattoria $$), mix of Premium/Popular tiers, each with vibe + reason + 'Ver partner' CTA. Italians 'Ver todos' action also rendered. No console errors."
    - agent: "testing"
