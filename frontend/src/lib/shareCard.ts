...
export function drawPassportFrame(ctx: CanvasRenderingContext2D) {
  ...
  halo.addColorStop(0.5, 'rgba(212,175,55,0.03)');
...
export function drawSeal(
  ...
  if (opts?.dashed) {
    ...
    fill.addColorStop(0, 'rgba(212,175,55,0.20)');
    fill.addColorStop(1, 'rgba(212,175,55,0.05)');
    ctx.fillStyle = fill;
    ctx.fill();
  ...
  ctx.fillStyle = opts?.dashed ? 'rgba(212,175,55,0.4)' : GOLD_BRIGHT;
