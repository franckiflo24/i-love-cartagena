...
ENRICH_FIELDS = (
    "google_place_id", "description", "phone", "instagram",
    "booking_link", "website", "address", "rating", "reviews",
)


@api_router.post("/admin/enrich")
async def admin_enrich(request: Request):
    """Batch-enrich partner fields. Accepts {secret, updates: [{partner_id, ...fields}]}"""
    body = await request.json()
    if body.get("secret") != os.environ.get("DEMO_SIGNUP_CODE", ""):
        raise HTTPException(403, "Invalid secret")
    updated = 0
    for u in body.get("updates", []):
        ...
        if not pid:
            continue
        fields = {k: u[k] for k in ENRICH_FIELDS if k in u and u[k]}
        if fields:
            res = await db.partners.update_one({"partner_id": pid}, {"$set": fields})
            if res.modified_count:
                updated += 1
    return {"updated": updated, "total": await db.partners.count_documents({})}


@api_router.post("/admin/dedup")
async def admin_dedup(request: Request):
    """Deduplicate partners by name. Keeps highest-scored record, merges richer fields."""
    body = await request.json()
    if body.get("secret") != os.environ.get("DEMO_SIGNUP_CODE", ""):
        raise HTTPException(403, "Invalid secret")
    ...
    content_fields = (
        ...
        "subcategory", "default_payment_link", "google_place_id",
    )
    tier_rank = {"elite": 4, "premium": 3, "popular": 2, "standard": 1, "institutional": 1}

    def _filled(v: Any) -> bool:
        return v is not None and v != "" and v != [] and v != {} and v != "null" and v != "?"

    def _score(p: dict) -> tuple:
        completeness = sum(1 for f in content_fields if _filled(p.get(f)))
        ...
        return (completeness, tier, rating, reviews, pid_priority)

    groups: dict[str, list] = {}
    async for doc in db.partners.find({}):
    ...
    for name, docs in dups.items():
        ...
        for loser in losers:
            for f in content_fields:
                if not _filled(winner.get(f)) and _filled(loser.get(f)):
                    merge_updates.setdefault(f, loser[f])
                if f in ("rating", "reviews") and _filled(loser.get(f)):
                    if float(loser.get(f, 0) or 0) > float(winner.get(f, 0) or 0):
                        merge_updates[f] = loser[f]

        if merge_updates:
            await db.partners.update_one({"partner_id": winner["partner_id"]}, {"$set": merge_updates})
            merged += len(merge_updates)
        ...
        r = await db.partners.delete_many({"partner_id": {"$in": loser_ids}})
        removed += r.deleted_count

    remaining = await db.partners.count_documents({})
    return {"duplicates_found": len(dups), "removed": removed, "fields_merged": merged, "remaining": remaining}


@api_router.delete("/admin/enrich")
async def remove_enrich_endpoint():
    """Self-documenting: this endpoint exists temporarily for data enrichment."""
    return {"message": "Use POST to enrich partner data"}
